...
StandardMaterial* MaterialManager::LoadFromMTLFile(GraphicsEngine* pGraphicsEngine, const std::string& filename, const std::string& materialName)
{
	...
	while (file >> read && readingTargetMaterial) {
		if (read == "map_Kd") {
			...
			LoadTexture(pResult->albedoTexture, DemoCore::imgFolderPath + texFilename, TextureType::COLOR);
		}
		else if (read == "map_Ks") {
			file >> texFilename;

			LoadTexture(pResult->specularTexture, DemoCore::imgFolderPath + texFilename, TextureType::COLOR);
		}
		else if (read == "map_Ns") {
			file >> texFilename;

			//TODO map_Ns is the the inverse of roughness!!
			LoadTexture(pResult->roughnessTexture, DemoCore::imgFolderPath + texFilename, TextureType::COLOR);
		}
		else if (read == "map_Bump" || read == "map_bump" || read == "bump") {
			file >> texFilename;

			LoadTexture(pResult->normalMap, DemoCore::imgFolderPath + texFilename, TextureType::DATA);
		}
		else if (read == "newmtl") {
